...
import('@/utils/referral').then(({ storeRef }) => {
  ...
  if (data.user) {
    ...
    if (refCode) {
      ...
      const { data: referrer } = await supabase.from('profiles')
        .select('id, referral_count, subscription_expires_at')
        .eq('referral_code', refCode)
        .maybeSingle()

      if (referrer && referrer.id !== data.user.id) {
        // Give referrer +7 free days on their subscription
        const newExpiry = new Date(referrer.subscription_expires_at || Date.now())
        ...
        await supabase.from('referrals').insert({
          ...
          reward_type: '7_free_days',
        }).catch(() => {})

        localStorage.removeItem('talenthub_ref')
      }
    }
  }
